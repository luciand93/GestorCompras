...
export function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  if (dbStatus === "checking") {
    return (
      <div className="min-h-screen bg-[#102213] flex items-center justify-center">
        <p className="text-[#13ec37]/80 text-sm">Comprobando conexión...</p>
      </div>
    );
  }
  ...
  if (dbStatus === "error") {
    return (
      <div className="min-h-screen bg-[#102213] text-white flex flex-col items-center justify-center p-6 safe-area-inset">
        <div className="max-w-sm w-full text-center space-y-4">
          ...
          <button
            type="button"
            onClick={retry}
            className="mt-4 px-6 py-3 rounded-lg bg-[#13ec37] text-[#102213] font-medium text-sm hover:bg-[#13ec37]/90 transition-colors"
          >
            Reintentar
          </button>
        </div>
